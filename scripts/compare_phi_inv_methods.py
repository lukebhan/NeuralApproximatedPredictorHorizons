...
for k in range(NUM_SAMPLES):
    ...
    try:
        ...
        print(f"[sample {k+1}] skipped due to inverse failure: {e}")
        continue
    ...
    if USE_FNO:
        ...
        max_diff_fno_vs_direct.append(
            np.max(np.abs(rho_fno - rho_direct_interp))
        )

     # Save the first successful example for plotting
    ...
    if (k + 1) % max(1, NUM_SAMPLES // 10) == 0:
        print(f"[{k+1:4d}/{NUM_SAMPLES}] processed")
...
# ============================================================
# Bar plot of aggregate average errors
# ============================================================
